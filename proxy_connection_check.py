...
async def collect_local_socket_details(local_socket_port = list()):
...
async def collect_top_ip_details(local_socket_port, top_inbound_ip = dict(), top_outbound_ip = dict()):
    with open(sys.argv[1], "r") as file:
        for line in file.read().split("\n"):
...
async def collect_top_port_details(local_socket_port, top_inbound_port = dict(), top_outbound_port = dict()):
    with open(sys.argv[1], "r") as file:
        for line in file.read().split("\n"):
            if re.search(r"[\w\s]+?([\d\.]{7,15}):([\d]{1,5})\s+([\d\.]{7,15}):([\d\*]{1,5})\s+ESTABLISHED", line):
                re_connection_pattern = re.search(r"[\w\s]+?([\d\.]{7,15}):([\d]{1,5})\s+([\d\.]{7,15}):([\d\*]{1,5})\s+ESTABLISHED", line);
...
async def collect_top_connection_details(local_socket_port, top_inbound_connection = dict(), top_outbound_connection = dict()):
    with open(sys.argv[1], "r") as file:
        for line in file.read().split("\n"):
            if re.search(r"[\w\s]+?([\d\.]{7,15}):([\d]{1,5})\s+([\d\.]{7,15}):([\d\*]{1,5})\s+ESTABLISHED", line):
                re_connection_pattern = re.search(r"[\w\s]+?([\d\.]{7,15}):([\d]{1,5})\s+([\d\.]{7,15}):([\d\*]{1,5})\s+ESTABLISHED", line);
...
async def main():
    local_socket_port = await collect_local_socket_details();
    top_inbound_ip, top_outbound_ip = await collect_top_ip_details(local_socket_port);
    top_inbound_port, top_outbound_port = await collect_top_port_details(local_socket_port);
    top_inbound_connection, top_outbound_connection = await collect_top_connection_details(local_socket_port);
    
    print("\n" + " Top Inbound IP ".center(180, "-") + "\n" , json.dumps(top_inbound_ip, indent=4));
    print("\n" + " Top Inbound Port ".center(180, "-") + "\n" , json.dumps(top_inbound_port, indent=4));
    print("\n" + " Top Inbound Connection ".center(180, "-") + "\n" , json.dumps(top_inbound_connection, indent=4));

    print("\n" + " Top Outbound IP ".center(180, "-") + "\n" , json.dumps(top_outbound_ip, indent=4));
    print("\n" + " Top Outbound Port ".center(180, "-") + "\n" , json.dumps(top_outbound_port, indent=4));
    print("\n" + " Top Outbound Connection ".center(180, "-") + "\n" , json.dumps(top_outbound_connection, indent=4));

if "__main__" in __name__:
    if len(sys.argv[1:]) != 1:
        print(f"\n[+] Usage: {sys.argv[0]} Device_Connection_File\n");
    else:
        asyncio.run(main());
